...
pub fn get_user_by_token(db: &DBEngine, token: &str) -> Result<Option<TokenOwner>, sqlite::Error> {
    ...
    match statement.next()? {
        sqlite::State::Row => {
            let professor_id: Option<i64> = statement.read::<Option<i64>, _>("professor_id")?;

            let student_id: Option<i64> = statement.read::<Option<i64>, _>("student_id")?;
...
pub fn delete_all_tokens(db: &DBEngine) -> Result<(), sqlite::Error> {
    let mut statement = db.run_query(
        "
...
pub fn delete_token(db: &DBEngine, token: &str) -> Result<(), sqlite::Error> {
    let mut statement = db.run_query(
        "
        DELETE FROM auth_tokens
        WHERE token = ?
