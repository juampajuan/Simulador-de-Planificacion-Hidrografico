...
pub fn get_my_simulations(request: &mut Request, db: Arc<Mutex<DBEngine>>) -> HandlerResult {
...
pub fn select_exam_simulation(request: &mut Request, db: Arc<Mutex<DBEngine>>) -> HandlerResult {
    ...
    match student_simulations::select_student_simulation_locked(&db, student_id, payload.simulation_id) {
        Ok(_) => generic::string_response("Estado de entrega actualizado con éxito".to_string(), 200),
