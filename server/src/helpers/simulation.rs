...
use crate::structs::request::{FullSimulationRequest, HandlerResult, RequestContext};
...
pub struct SimulationImagesInput {
    pub matrix: DepthMatrix,
    pub interpolation: Vec<Vec<f64>>,
    pub path: Vec<(usize, usize)>,
    pub params: StudentMeasuringParameters,
...
pub fn save_simulation_images(
    ...
    let save = |categoria: &str, bytes: &[u8]| -> Option<String> {
        let filename = format!(
            "{}_{}_{}_{}.png",
            base, categoria, input.student_id, input.attempt_number
        );
        let file_path = format!("{}/images/{}", input.settings.storage_path, filename);
        match std::fs::write(&file_path, bytes) {
            ...
            Err(e) => {
                send_message_to_logger(
                    tx,
                    format!(
                        "No se pudo guardar el PNG de {} en storage ({}): {}",
                        categoria, file_path, e
                    ),
                    LogType::Error,
                );
                None
    ...
    let map_saved = save("simulacion", &input.map_bytes);

    let (coverage_image, _min, _max) = simulations::create_simulation_with_coverage(
        &input.matrix,
        &input.interpolation,
        &input.path,
        input.params,
    ...
    let _ = coverage_image.write_to(
        &mut Cursor::new(&mut coverage_bytes),
        image::ImageFormat::Png,
    );
    let coverage_saved = save("cobertura", &coverage_bytes);

    let difference_matrix = simulations::generate_difference_matrix(
        &input.matrix,
        input.interpolation.clone(),
        &log_debug,
    );
    let difference_image =
        simulations::create_difference_png(&input.matrix, difference_matrix, &log_debug);
    let mut difference_bytes = Vec::new();
    let _ = difference_image.write_to(
        &mut Cursor::new(&mut difference_bytes),
        image::ImageFormat::Png,
    );
...
pub fn lock_get_or_create_matrix(
    ...
    let m = simulations::create_depth_matrix(file_path, &log_debug)?;
...
pub fn lock_get_or_create_path(
    cache: &Arc<Mutex<FileCache>>,
    cache_key: i64,
    matrix: &DepthMatrix,
    ...
    if let Some(path_coor) = lock.get_path_if_valid(cache_key, path_params) {
        ...
        let mut lock = match cache.lock() {
            ...
            Err(_) => {
                return Err(
                    "Error interno: no se pudo acceder al cache (mutex corrupto)".to_string(),
                );
            }
        };
