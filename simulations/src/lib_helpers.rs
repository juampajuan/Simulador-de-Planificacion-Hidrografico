...
use crate::processing::measuring::{
    beam::{
        calculate_covered_radius, get_perpendicular_measurements, get_points_circular_to_this,
        get_points_in_radius,
    },
    find_measuring_points,
};
...
pub(super) fn get_covered_points(
    ...
    let points_to_measure = find_measuring_points(path, distance_between_points, matrix);
    ...
    match params.echo_sounder_parameters.mode {
        EcosondaMode::Monohaz => {
            ...
            for &point in &points_to_measure {
                let circle_points = match uniform_radius {
                    ...
                    None => get_points_circular_to_this(
                        &point,
                        params.echo_sounder_parameters.angle,
                        matrix,
                    ),
        ...
        EcosondaMode::Multihaz => {
            // Mismo criterio que Monohaz: sin profundidad real, todos los
            // puntos usan la profundidad PROMEDIO (ancho de swath uniforme).
            let fixed_depth = if use_real_depth {
                None
            } else {
                Some(get_matrix_avg_depth(matrix).unwrap_or(0.0))
            };

            let measurements = get_perpendicular_measurements(
                &points_to_measure,
                constants.echosounder.multihaz_angle_deg,
                fixed_depth,
                matrix,
            );
            match measurements {
                ...
                //Aca nunca va a entrar, pero sino se queja el compilador
