...
pub fn create_matrix_with_measurments_and_eliminate_none_points(
    measurements: &Vec<((usize, usize), Option<f64>)>,
    geotiff: &DepthMatrix,
) -> (Vec<(usize, usize)>, Vec<Vec<f64>>) {
    let mut matrix_with_measured_points: Vec<Vec<f64>> =
        vec![vec![0.0f64; geotiff.width]; geotiff.height];
