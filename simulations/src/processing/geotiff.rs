...
>;
pub type GeotiffCoordinates =
    Result<((f64, f64), (f64, f64), (f64, f64), (f64, f64), (f64, f64)), gdal::errors::GdalError>;
...
fn load_geotiff(path: &str, log_debug: &dyn Fn(&str)) -> LoadGeotiffResult {
    ...
    let spatial_ref = SpatialRef::from_definition(&projection).map_err(|_| {
        gdal::errors::GdalError::BadArgument(
            "No se pudo leer el sistema de coordenadas".to_string(),
        )
    })?;
    ...
    let buffer: Buffer<f64> = raster_band.read_as((0, 0), (cols, rows), (cols, rows), None)?;

    log_debug(&format!(
        "GeoTIFF cargado: {}x{} píxeles, resolución: {}, no_data: {:?}",
        cols, rows, res_x, no_data
    ));
    Ok((
        buffer,
        cols,
...
pub fn processing_geotiff(
    path: &str,
    log_debug: &dyn Fn(&str),
) -> Result<DepthMatrix, gdal::errors::GdalError> {
    let (buffer, cols, rows, no_data_value, size_x, size_y, geo_transform, projection) =
        load_geotiff(path, log_debug)?;
