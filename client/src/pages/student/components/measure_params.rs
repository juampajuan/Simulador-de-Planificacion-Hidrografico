...
pub fn measures_params(props: &MeasuresProps) -> Html {
    ...
    // Usamos los handles que vienen por props
    ...
    let is_form_complete = !state.boat.trim().is_empty() && 
        [
            ...
            &state.echosounder_velocity, &state.umbral, &state.frecuencia
    ...
    let render_check = |label: &'static str, value: bool, id: &'static str| {
        ...
        html! {
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={value} onchange={Callback::from(move |e: Event| {
                    let input: HtmlInputElement = e.target_unchecked_into();
                    let mut s = (*state).clone();
                    match id { 
                        "m" => s.uses_mathegapher = input.checked(), 
                        "s" => s.uses_sound_profiler = input.checked(), 
                        _ => s.uses_inertial_sensor = input.checked() 
                    };
                    state.set(s);
                })} /> {label}
            </label>
            ...
            <div class="p-3">
                ...
                <div class="flex flex-col gap-4">
                    {for vec![
                        ...
                        ("intervalo", "Intervalo de repetición del pulso"), ("frecuencia", "Frecuencia"), 
                        ("velocidad", "Velocidad del sonido"), ("longitud", "Longitud del pulso"), 
                        ("potencia", "Potencia transmitida"), ("ganancia", "Ganancia"), ("umbral", "Umbral")
                    ].into_iter().map(|(id, l)| {
                        let state = state.clone();
                        let current_val = match id {
                            "min_limit" => state.min_limit.clone(), "max_limit" => state.max_limit.clone(),
                            "intervalo" => state.pulse_repetition_interval.clone(), "frecuencia" => state.frecuencia.clone(),
                            "velocidad" => state.echosounder_velocity.clone(), "longitud" => state.pulse_length.clone(),
                            "potencia" => state.transmited_potency.clone(), "ganancia" => state.gain.clone(),
                        ...
                        html! { 
                            <input type="number" placeholder={l} class={input_cls} value={current_val}
                                oninput={Callback::from(move |e: InputEvent| { 
                                    ...
                                    match id { 
                                        "min_limit" => s.min_limit = v, "max_limit" => s.max_limit = v, 
                                        "intervalo" => s.pulse_repetition_interval = v, "frecuencia" => s.frecuencia = v, 
                                        "velocidad" => s.echosounder_velocity = v, "longitud" => s.pulse_length = v,
                                        "potencia" => s.transmited_potency = v, "ganancia" => s.gain = v, _ => s.umbral = v 
                                    };
                                    state.set(s);
                                })} 
                            /> 
